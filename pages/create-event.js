...
export default function CreateEvent() {
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    //Create body object to pass to storage
    const body = {
      name:eventName,
    ...
    //Call storage api to store data on IPFS and then createEvent which calls the contrac
    try{
  ...
  //Calls contract to update chain with data + CID to ipfs for look up
  const createEvent = async (cid) =>{
    try{
      ...
      if(rsvpContract){
        ...
        setSuccess(true);
        setLoading(false);
        setMessage("Your event has been create successfully.");
      } else{
